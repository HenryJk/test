...
__global__ void mm_kernel(matrix a, matrix b, matrix result, int size)
{
	...
	int blockArea = (blockDim.x+1) * blockDim.y;
	int indexA, indexB, index, k;
	
	indexA = tidx * (blockDim.x+1) + tidy;
	...
	for (index = 0; index < (size+blockDim.x-1)/blockDim.x; index++) {
		...
		for(k = 0; k < blockDim.y; k++) {
			c += sdata[tidx * (blockDim.x+1) + k] * sdata[blockArea + k * (blockDim.x+1) + tidy];
		}
		__syncthreads();
	}
...
void work()
{
	...
	before = wall_clock_time();
	mm_kernel<<<grid, block, 2*(BLOCKSIZE+1)*BLOCKSIZE*sizeof(float)>>>(a, b, result2, size);
